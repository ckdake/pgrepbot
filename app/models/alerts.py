...
class AlertThreshold(BaseModel, RedisModelMixin):
    ...
    evaluation_window_seconds: int = Field(default=300, description="Time window for evaluation in seconds")
    consecutive_breaches: int = Field(default=2, description="Number of consecutive breaches before alerting")
...
class AlertRule(BaseModel, RedisModelMixin):
    ...
    # Actions
    notification_channels: list[str] = Field(default_factory=list, description="Notification channels to use")
