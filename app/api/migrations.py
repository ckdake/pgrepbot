    ...
    try:
        ...
        for i, db_config in enumerate(target_databases):
            try:
                ...
                result = await _execute_migration_on_database(
                    db_config, request.sql_script, request.dry_run
                )
        ...
        await _store_migration_history(
            execution_id, user.username, request, results, 
            successful_count, failed_count, execution_time_ms, redis_client
        )
